-- Clean up duplicate menu items and keep only those with add-ons
-- This migration removes duplicates and keeps items that have add-ons

-- First, let's see what we have and clean up duplicates
-- Remove all existing menu items to start fresh
DELETE FROM add_ons;
DELETE FROM variations;
DELETE FROM menu_items;

-- Now insert only the essential menu items that should have add-ons
-- Coffee (22oz) items
...
FROM menu_items mi 
WHERE mi.name IN ('Creamy Buldak Overload', 'Cheesy Pancit Canton', 'Cheesy Fries Overload', 'Cheesy Nachos Overload');
